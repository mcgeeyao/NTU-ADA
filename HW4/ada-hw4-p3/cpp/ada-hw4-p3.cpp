...
vector<int> insert(int p, int id) {
    ...
    while (left >= 0 && inconceal[left] == true) left--;
    while (right < inconceal.size() && inconceal[right] == true) right++;
...
vector<int> remove(int p) {
    ...
    if (inconceal[p]) {
        int tmp = 0;
        while (tmp<conceal.size() && conceal[tmp]!=id) tmp++;
        conceal.erase(conceal.begin() + tmp);

        int pid = pairs[p];

        int pairpos = 0;
        while (pairpos < origin.size() && origin[pairpos] != pid) pairpos++;
        int pairfind = 0;
        while (pairfind < conceal.size() && conceal[pairfind] != pid) pairfind++;
